		...
		do {
			if (ts_pwr[i].type == 1) {
				...
				if (ts->vdd_regulator[i] != NULL && !IS_ERR(ts->vdd_regulator[i])) {
					...
					/*
					TOUCH_POWER_MSG("power[%d]: regulator[%s] disabled\n", i,
...
static int mms_set_active_area(struct mms_data* ts, u8 mode)
{
	...
	if (mode) {
		...
		if(i2c_master_send(ts->client,write_buf,10)!=10) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_ACTIVE_AREA = X1 : %d  | Y1 : %d | X2 : %d | Y2 : %d \n",
				((write_buf[2] << 8) | write_buf[3]),
				((write_buf[4] << 8) | write_buf[5]),
				((write_buf[6] << 8) | write_buf[7]),
				((write_buf[8] << 8) | write_buf[9]));
		}
...
static int tci_control(struct mms_data* ts, int type, u16 value)
{
	...
	case IDLE_REPORTRATE_CTRL:
		...
		if(i2c_master_send(ts->client,write_buf,3)!=3) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_IDLE_REPORTRATE_REG = %d \n", write_buf[2]);
		}
		break;
	case ACTIVE_REPORTRATE_CTRL:
		...
		if(i2c_master_send(ts->client,write_buf,3)!=3) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_ACTIVE_REPORTRATE_REG = %d \n", write_buf[2]);
		}
		break;
	...
	case TOUCH_SLOP_CTRL:
		...
		if(i2c_master_send(ts->client,write_buf,3)!=3) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_TOUCH_SLOP_REG = %d \n", write_buf[2]);
		}
		break;
	case TAP_MIN_DISTANCE_CTRL:
		...
		if(i2c_master_send(ts->client,write_buf,3)!=3) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_TAP_MIN_DISTANCE_REG = %d \n", write_buf[2]);
		}
		break;
	case TAP_MAX_DISTANCE_CTRL:
		...
		if(i2c_master_send(ts->client,write_buf,3)!=3) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_TAP_MAX_DISTANCE_REG = %d \n", write_buf[2]);
		}
		break;
	case MIN_INTERTAP_CTRL:
		...
		if(i2c_master_send(ts->client,write_buf,4)!=4) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_MIN_INTERTAP_REG = %d\n", ((write_buf[2] << 8) | write_buf[3]));
		}
		break;
	case MAX_INTERTAP_CTRL:
		...
		if(i2c_master_send(ts->client,write_buf,4)!=4) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_MAX_INTERTAP_REG = %d\n", ((write_buf[2] << 8) | write_buf[3]));
		}
		break;
	case TAP_COUNT_CTRL:
		write_buf[0] = MIT_REGH_CMD;
		...
		if(i2c_master_send(ts->client,write_buf,3)!=3) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_TAP_COUNT_REG = %d \n", write_buf[2]);
		}
		break;
	case INTERRUPT_DELAY_CTRL:
		...
		if(i2c_master_send(ts->client,write_buf,4)!=4) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_INTERRUPT_DELAY_REG = %d\n", ((write_buf[2] << 8) | write_buf[3]));
		}
		break;
	/* TCI2 reg */
	...
	case TOUCH_SLOP_CTRL2:
		...
		if(i2c_master_send(ts->client,write_buf,3)!=3) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_TOUCH_SLOP_REG2 = %d \n", write_buf[2]);
		}
		break;
	case TAP_MIN_DISTANCE_CTRL2:
		...
		if(i2c_master_send(ts->client,write_buf,3)!=3) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_TAP_MIN_DISTANCE_REG2 = %d \n", write_buf[2]);
		}
		break;
	case TAP_MAX_DISTANCE_CTRL2:
		...
		if(i2c_master_send(ts->client,write_buf,3)!=3) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_TAP_MAX_DISTANCE_REG2 = %d \n", write_buf[2]);
		}
		break;
	case MIN_INTERTAP_CTRL2:
		...
		if(i2c_master_send(ts->client,write_buf,4)!=4) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_MIN_INTERTAP_REG2 = %d\n", ((write_buf[2] << 8) | write_buf[3]));
		}
		break;
	case MAX_INTERTAP_CTRL2:
		...
		if(i2c_master_send(ts->client,write_buf,4)!=4) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_MAX_INTERTAP_REG2 = %d\n", ((write_buf[2] << 8) | write_buf[3]));
		}
		break;
	...
	case INTERRUPT_DELAY_CTRL2:
		...
		if(i2c_master_send(ts->client,write_buf,4)!=4) {
			...
			TOUCH_INFO_MSG("MIT_LPWG_INTERRUPT_DELAY_REG2 = %d\n", ((write_buf[2] << 8) | write_buf[3]));
		}
		break;
...
static int lpwg_control(struct mms_data* ts, u8 mode)
{
	...
	case LPWG_DOUBLE_TAP:                         /* Only TCI-1 */
		tci_control(ts, IDLE_REPORTRATE_CTRL, ts->pdata->tci_info->idle_report_rate); /* 10 Hz */
		tci_control(ts, ACTIVE_REPORTRATE_CTRL, ts->pdata->tci_info->active_report_rate);
		tci_control(ts, SENSITIVITY_CTRL, ts->pdata->tci_info->sensitivity); /* Threshold */

		tci_control(ts, TCI_ENABLE_CTRL, 1);  /* Tci-1 enable */
		tci_control(ts, TOUCH_SLOP_CTRL, ts->pdata->tci_info->touch_slope); /* touch_slop = 10mm */
		tci_control(ts, TAP_MIN_DISTANCE_CTRL, ts->pdata->tci_info->min_distance); /* Minimum tap distance = 0mm */
		tci_control(ts, TAP_MAX_DISTANCE_CTRL, ts->pdata->tci_info->max_distance); /* Maximum tap distance = 10mm */
		tci_control(ts, MIN_INTERTAP_CTRL, ts->pdata->tci_info->min_intertap); /* min inter_tap = 0ms */
		tci_control(ts, MAX_INTERTAP_CTRL, ts->pdata->tci_info->max_intertap); /* max inter_tap */
		tci_control(ts, TAP_COUNT_CTRL, ts->pdata->tci_info->tap_count);   /* tap count = 2 */
		tci_control(ts, INTERRUPT_DELAY_CTRL, 0); /* interrupt delay = 0ms */
		tci_control(ts, TCI_ENABLE_CTRL2, 0); /* Tci-2 disable */
		break;
	case LPWG_MULTI_TAP:                           /* TCI-1 and TCI-2 */
		tci_control(ts, IDLE_REPORTRATE_CTRL, ts->pdata->tci_info->idle_report_rate); /* 10 Hz */
		tci_control(ts, ACTIVE_REPORTRATE_CTRL, ts->pdata->tci_info->active_report_rate);
		tci_control(ts, SENSITIVITY_CTRL, ts->pdata->tci_info->sensitivity); /* Threshold */
		...
		tci_control(ts, TOUCH_SLOP_CTRL2, ts->pdata->tci_info->touch_slope_2); /* touch_slop = 10mm */
		tci_control(ts, TAP_MIN_DISTANCE_CTRL2, ts->pdata->tci_info->min_distance_2); /* Minimum tap distance = 0mm */
		tci_control(ts, TAP_MAX_DISTANCE_CTRL2, ts->pdata->tci_info->max_distance_2); /* Maximum tap distance = MAX */
		tci_control(ts, MIN_INTERTAP_CTRL2, ts->pdata->tci_info->min_intertap_2); /* min inter_tap = 0ms */
		tci_control(ts, MAX_INTERTAP_CTRL2, ts->pdata->tci_info->max_intertap_2); /* max inter_tap = 700ms */
		tci_control(ts, TAP_COUNT_CTRL2, ts->pdata->tap_count);   /* tap count = user_setting */
		tci_control(ts, INTERRUPT_DELAY_CTRL2, ts->pdata->tci_info->interrupt_delay_2); /* interrupt delay = 0ms */
		break;
	default:
...
static ssize_t mms_self_diagnostic_show(struct i2c_client *client, char *buf)
{
	...
	if (ret < 0) {
		...
		memset(buf, 0, PAGE_SIZE);
		ret = snprintf(buf, PAGE_SIZE, "failed to get open short data\n\n");
	...
	if (ret < 0) {
		...
		memset(buf, 0, PAGE_SIZE);
		ret = snprintf(buf, PAGE_SIZE, "failed to get slope data\n\n");
		ts->pdata->selfdiagnostic_state[2] = 0;
	}
	write_file(sd_path, buf, 0);
	msleep(30);
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "MAX - MIN = %d\n", max - min);
	if (ts->module.otp == OTP_NOT_SUPPORTED) {
	...
	if(buf1 != NULL && !strcmp(buf1,"update_all")) {
		...
		if (ts->pdata->panel_on == 0 && ts->pdata->lpwg_panel_on == 0) {
			...
			if (pre_lpwg_prox_status == ts->pdata->lpwg_prox) {
				TOUCH_INFO_MSG("%s, Skip LPWG setting!! - us10 - 0714 - \n",__func__);
