...
int SW3800_Authentication(void)
{
	...
	vendor_code = bifTransReadUint();
	printk("%s : [Slide] vendor_code = %x \n ", __func__, vendor_code);

	if(vendor_code == 0x01)
		vendor = COVER_SW3800_VC1;
	else if(vendor_code == 0x02)
		vendor = COVER_SW3800_VC2;
	else if(vendor_code == 0x03)
		vendor = COVER_SW3800_VC3;
	else if(vendor_code == 0x04)
		vendor = COVER_SW3800_VC4;
	else{
		set_cover_id_pin_output_mode();
		set_cover_id_pin_low();
		printk("%s : [Slide] COVER_SW3800_VENDOR_ERR\n ", __func__);
		return COVER_SW3800_VENDOR_ERR;
	}

	set_cover_id_pin_output_mode();
	Apply_SHA1(0x0, 0x0,&ci);			//user sectet key 64bit
	udelay(200);
